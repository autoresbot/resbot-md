...
async function handle(sock, messageInfo) {
  ...
  try {
    // Dulu node IQ mentah dikirim lalu dibaca lewat res.content[0].attrs.
    // Bentuk node hasil decode zapo berbeda, sehingga id & subject terbaca
    // undefined dan sewa tersimpan sebagai "undefined@g.us". Sekarang dipakai
    // parser resmi zapo yang mengembalikan objek bertipe.
    const infoGrub = await sock.groupGetInviteInfo(result_sewa);

    if (!infoGrub?.jid) {
      throw new Error("Link grup tidak valid atau info grup tidak terbaca");
    }
