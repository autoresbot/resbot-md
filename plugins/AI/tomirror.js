const DEFAULT_PROMPT = `Recreate this image as a mirror selfie taken with an iPhone 17 Pro Max. The subject should be holding the phone as if taking a photo of themselves in the mirror, with the phone's flash clearly visible and reflecting off the mirror to create a dramatic and aesthetic lighting effect. Ensure the subject's facial expression, posture, and clothing remain exactly the same as the original image. The background behind the subject should be replaced with a clean, white curtain.`;
...
async function handle(sock, messageInfo) {
  ...
  try {
    ...
    const createRes = await http.get('https://api.autoresbot.com/api/ai-image', {
      params: { url: imageUrl, prompt: DEFAULT_PROMPT },
      headers: {
