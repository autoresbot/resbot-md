...
export function createBaileysCompatSocket(client, deps = {}) {
  ...
  sock.readMessages = async (keys) => {
    ...
    // zapo mengelompokkan receipt per chat; Baileys menerima daftar key campuran.
    const perChat = new Map();
    for (const key of keys) {
      if (!key?.remoteJid || !key?.id) continue;
      const list = perChat.get(key.remoteJid) ?? [];
      list.push(key.id);
      perChat.set(key.remoteJid, list);
    }
    for (const [jid, ids] of perChat) {
