...
/**
 * Kosongkan daftar absen sebuah grup (.resetabsen).
 * @returns {number} jumlah orang yang tadinya sudah absen
 */
async function resetAbsen(id) {
  try {
    const data = await findAbsen(id);
    const jumlah = Array.isArray(data?.member) ? data.member.length : 0;
    if (jumlah === 0) return 0;

    await updateAbsen(id, { member: [] });
    return jumlah;
  } catch (error) {
    console.error(`[resetAbsen] Error reset absen "${id}":`, error);
    return 0;
  }
}

/**
 * Cocokkan satu orang dengan daftar absen LEWAT NOMORNYA.
 *
 * Absen lama tersimpan sebagai nomor HP (628xx@s.whatsapp.net), sedangkan grup
 * ber-alamat LID mengenali peserta lewat `id` berupa @lid. Membandingkan
 * string mentah membuat orang yang sudah absen tetap dianggap belum absen.
 * Di sini SEMUA bentuk identitas ikut dibandingkan, jadi data lama & baru
 * sama-sama terbaca.
 *
 * @param {string[]} members isi data.member
 * @param {...string} identitas jid/nomor yang mewakili satu orang
 */
function sudahAbsen(members, ...identitas) {
  const target = identitas.map(nomorDari).filter(Boolean);
  if (target.length === 0) return false;
  return (members || []).some((m) => target.includes(nomorDari(m)));
}

/** Apakah peserta grup ini sudah absen? (cek lewat semua identitasnya) */
function pesertaSudahAbsen(members, peserta) {
  return sudahAbsen(members, peserta?.id, peserta?.jid, peserta?.lid, peserta?.phoneNumber);
}
