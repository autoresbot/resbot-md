...
const validations = [
  ...
  {
    key: 'pairing_code',
    validate: (value) => !value || isPairingCodeValid(value),
    errorMessage: () => pairingCodeErrorMessage(config.pairing_code),
  },
];

/**
 * Alfabet kode pairing WhatsApp (Crockford base32).
 *
 * Sengaja TIDAK memuat `0`, `I`, `O`, dan `U` — karakter yang mudah tertukar
 * saat dibaca/diketik (`O` vs `0`, `I` vs `1`, `U` vs `V`). Jadi kode seperti
 * "AUTORES" ditolak bukan karena salah ketik, tapi karena `O` dan `U` memang
 * tidak ada di alfabetnya.
 */
const PAIRING_ALPHABET = '123456789ABCDEFGHJKLMNPQRSTVWXYZ';
const PAIRING_CODE_LENGTH = 8;

/** Bentuk kode yang sebenarnya divalidasi zapo: tanpa '-', huruf besar. */
...
function isPairingCodeValid(value) {
  const kode = normalizePairingCode(value);
  if (kode.length !== PAIRING_CODE_LENGTH) return false;
  return [...kode].every((huruf) => PAIRING_ALPHABET.includes(huruf));
}

/**
 * Pesan error yang menyebut MASALAHNYA, bukan cuma "tidak valid".
 *
 * Divalidasi saat startup supaya ketahuan sebelum koneksi dibuka — sebelumnya
 * kesalahan baru muncul setelah handshake, sebagai "Gagal meminta kode pairing".
 */
function pairingCodeErrorMessage(value) {
  const kode = normalizePairingCode(value);
  const baris = [`PAIRING_CODE "${value}" tidak valid.`];

  if (kode.length !== PAIRING_CODE_LENGTH) {
    baris.push(`  • Harus tepat ${PAIRING_CODE_LENGTH} karakter, punyamu ${kode.length}.`);
  }

  const salah = [...new Set([...kode].filter((h) => !PAIRING_ALPHABET.includes(h)))];
  if (salah.length) {
    baris.push(`  • Karakter tidak diizinkan: ${salah.map((h) => `"${h}"`).join(', ')}`);
    baris.push(`  • Yang boleh: ${PAIRING_ALPHABET}`);
    baris.push('  • Perhatikan: 0, I, O, dan U TIDAK boleh (mudah tertukar saat dibaca).');
  }

  baris.push('  • Kosongkan PAIRING_CODE kalau ingin kodenya dibuat otomatis.');
  return baris.join('\n');
}
...
// Upload tmp file
/**
 * @deprecated Pakai `uploadImageFile()` dari lib/uploader.js.
 *
 * Fungsi ini tidak lagi dipakai di mana pun. Ia menyimpan bug yang sama dengan
 * `api.tmpUpload()`: `fs.createReadStream(path)` membuat form-data menebak
 * content-type DARI EKSTENSI FILE, padahal `getMediaExtension()` selalu memberi
 * `.jpg` untuk setiap imageMessage WhatsApp walau isi aslinya WebP/PNG.
 * Server lalu menolak dengan "Isi file tidak sesuai dengan format yang diizinkan."
 *
 * Dibiarkan apa adanya (tidak dihapus) agar plugin custom milik pengguna yang
 * mungkin memanggilnya tidak ikut rusak — tapi JANGAN dipakai untuk kode baru.
 */
 ...
 * @param {object} sock - Instance socket bot
...
async function convertToJid(sock, input) {
  try {
    ...
    // Cache lidMapping (diisi dari pesan masuk + metadata grup) -> nomor telepon
...
async function clearDirectory(dirPath) {
  try {
    ...
    // Sub-folder (mis. tmp/logs milik logger winston) sengaja dilewati: unlink
    // memang tidak bisa menghapus folder, dan dulu setiap percobaannya memunculkan
    // "⚠️ Gagal hapus: logs" di console pada tiap startup & jadwal pembersihan.
    ...
    for (const file of files) {
      try {
        await fs.promises.unlink(path.join(dirPath, file.name));
        successCount++;
      } catch {
        // File yang sedang dipakai proses lain (umum di Windows) cukup dilewati;
        // akan terhapus pada pembersihan berikutnya.
      }
    }
    ...
    if (error?.code === 'ENOENT') return; // folder belum ada, tidak ada yang perlu dibersihkan
...
function deleteFolderRecursive(folderPath) {
  // Pengaman: pemanggilnya membentuk path lewat path.join('./session/', id).
  // Kalau `id` kosong, hasilnya adalah folder sesi UTAMA — dan seluruh sesi bot
  // ikut terhapus. Di era Baileys itu masih bisa dipulihkan dari creds.json;
  // di zapo semuanya ada di satu file sqlite, jadi hilang berarti pairing ulang.
  const resolved = path.resolve(folderPath);
  const mainSession = path.resolve(process.cwd(), 'session');
  if (resolved === mainSession) {
    console.warn('[SAFETY] Menolak menghapus folder sesi utama:', resolved);
    return;
  }
 ...
 *
 * Menerima objek pesan penuh ({ message: {...} }) maupun objek message content
...
/**
 * Bersihkan nama file media yang berasal dari pengirim pesan.
 *
 * `mediaMessage.fileName` sepenuhnya dikendalikan oleh client pengirim,
 * sehingga tidak boleh dipakai apa adanya:
 *  - komponen direktori ("../", "C:\") bisa menulis file di luar tmp/database,
 *  - karakter meta shell bisa keluar dari tanda kutip pada plugin yang
 *    menyusun perintah ffmpeg lewat `exec()` (mis. TOOLS/voicechanger.js).
 *
 * Hanya karakter yang aman dan lazim untuk nama file yang dipertahankan.
 * Bila tidak tersisa apa pun yang valid, pemanggil memakai nama default.
 *
 * @param {string} name nama file mentah dari pesan
 * @returns {string} nama file yang aman (bisa string kosong)
 */
function sanitizeMediaFileName(name) {
  if (!name || typeof name !== 'string') return '';

  // Ambil basename saja -> buang seluruh komponen direktori (POSIX & Windows)
  const base = name.split(/[/\\]/).pop() || '';

  const safe = base
    .replace(/[\u0000-\u001f\u007f]/g, '') // karakter kontrol
    .replace(/[^a-zA-Z0-9._-]/g, '_') // hanya sisakan karakter aman
    .replace(/^\.+/, '') // cegah nama tersembunyi / ".."
...
function getMediaExtension(mediaType, mediaMessage) {
  switch (mediaType) {
    ...
    case 'sticker':
      return '.webp';
    case 'document': {
      // Ekstensi juga berasal dari nama file kiriman -> ikut disanitasi.
      const raw = path.extname(sanitizeMediaFileName(mediaMessage?.fileName));
      return raw && raw !== '.' ? raw : '.bin';
    }
    default:
      return '.bin';
  }
 ...
 *
 * Strategi:
 *  1) downloadBytes atas Proto.IMessage-nya.
 *  2) Kalau gagal, minta WhatsApp meng-upload ulang media (URL bisa
 *     kedaluwarsa), lalu unduh lagi dengan directPath yang baru.
 *
 * @param {object} fullMessage  Objek pesan lengkap { key, message } untuk media ini.
...
async function downloadMediaToFile(fullMessage, mediaType, mediaMessage, folderUse, logLabel) {
  ...
  // PENTING: zapo memilih bentuk sumber dengan `'rawNode' in source` —
  // objek `{ key, message }` gaya Baileys TIDAK punya `rawNode`, sehingga
  // seluruh objek itu dianggap sebagai Proto.IMessage dan media-nya tidak
  // ketemu ("message has no downloadable media"). Jadi yang dioper ke
  // download HARUS proto message-nya, bukan pembungkusnya.
  const protoMessage = fullMessage?.message ?? fullMessage;

  const buffer = await retryRequest(
    async () => {
      let buf;
      try {
        if (!sock?.zapo) throw new Error('Client zapo belum siap');
        buf = Buffer.from(await sock.zapo.message.downloadBytes(protoMessage));
      } catch (primaryErr) {
        // Recovery: minta re-upload lalu unduh ulang dengan directPath baru.
        // Berbeda dengan download, requestMediaReupload justru menuntut
        // pembungkus `{ key, message }` (ia memeriksa `'key' in source`).
        const retry = await sock.zapo.message
          .requestMediaReupload(fullMessage)
          .catch(() => null);

        if (!retry?.directPath || !mediaMessage) throw primaryErr;

        // directPath baru ditulis ke objek media yang memang bagian dari
        // protoMessage, lalu `url` lama dibuang agar tidak dipakai lagi.
        mediaMessage.directPath = retry.directPath;
        delete mediaMessage.url;
  ...
  // Nama file dari pesan tidak dipercaya -> selalu disanitasi lebih dulu.
  let fileName = sanitizeMediaFileName(mediaMessage?.fileName) || `${mediaType}_${Date.now()}`;
...
/**
 * Hasil langkah sebelumnya dalam rantai command (mis. `.removebg + sticker`).
 *
 * Runner rantai (lib/rantaiCommand.js) menulis hasil langkah sebelumnya ke
 * folder tmp/ lalu memasang nama filenya di `message.__mediaRantai`. Karena
 * kedua fungsi download di bawah memang mengembalikan NAMA FILE di tmp/,
 * cukup dikembalikan apa adanya di sini - dan SELURUH plugin media berjalan
 * tanpa perlu diubah sama sekali.
 */
function mediaRantai(message) {
  const nama = message?.__mediaRantai;
  return typeof nama === 'string' && nama ? nama : null;
}
 ...
 *
 * @param {object} message    Objek message penuh ({ message, key }).
 * @param {boolean} folderPath true -> simpan ke folder database, default -> tmp.
 */
async function downloadQuotedMedia(message, folderPath) {
  const folderUse = folderPath ? DatabaseFolder : tmpFolder;

  const dariRantai = mediaRantai(message);
  if (dariRantai) return dariRantai;
...
async function downloadMedia(message, folderPath) {
  ...
  try {
    ...
    // Pembungkus { key, message } dipertahankan karena requestMediaReupload
    // membutuhkannya; downloadMediaToFile yang memilah mana yang dipakai.
...
async function getName(sock, jid) {
  try {
    ...
    if (jid.endsWith('@g.us')) {
      ...
      // Lewat cache bersama: menghindari query berulang untuk grup yang sama.
      const { getGroupMetadata } = await import('./cache.js');
      const groupMetadata = await getGroupMetadata(sock, jid);
      if (!groupMetadata) return 'Unknown Group';
...
/**
 * Ubah `@<digit>` di dalam teks menjadi JID lengkap yang BENAR.
 *
 * Masalahnya: teks cuma memuat digit, tanpa domain. Cara lama menebak domain
 * dari `senderType` — tipe PENGIRIM PERINTAH — padahal yang di-tag orang lain.
 * Kalau yang di-tag beralamat LID sementara pengirim memakai nomor telepon,
 * JID-nya jadi `<lid>@s.whatsapp.net` dan WhatsApp menampilkannya sebagai nomor
 * asing (mis. `@+225 764989489187`), bukan tag.
 *
 * Solusinya: untuk chat grup, digit dicocokkan ke peserta grup yang sebenarnya
 * (`id`, `phoneNumber`, dan `lid`), lalu JID milik peserta itu dipakai apa
 * adanya. Di luar grup — atau kalau digitnya tidak ada di daftar peserta —
 * barulah jatuh ke tebakan lama.
 */
async function resolveMentionJids(sock, remoteJid, text, senderType) {
  const digits = [...text.matchAll(/@(\d{5,16})/g)].map((m) => m[1]);
  if (!digits.length) return [];

  const byDigits = new Map();
  if (typeof remoteJid === 'string' && remoteJid.endsWith('@g.us') && sock) {
    try {
      // Dinamis: cache.js mengimpor utils.js, jadi impor statis akan melingkar.
      const { getGroupMetadata } = await import('./cache.js');
      const metadata = await getGroupMetadata(sock, remoteJid);
      for (const p of metadata?.participants ?? []) {
        for (const jid of [p.phoneNumber, p.id, p.lid]) {
          if (typeof jid !== 'string') continue;
          const key = jid.split('@')[0];
          // Yang pertama menang: phoneNumber lebih dulu supaya tag menampilkan
          // nomor asli bila tersedia.
          if (!byDigits.has(key)) byDigits.set(key, jid);
        }
      }
    } catch {
      // Metadata gagal diambil -> pakai tebakan lama saja.
    }
  }

  const domainCadangan = senderType === 'lid' ? '@lid' : '@s.whatsapp.net';
  return digits.map((d) => byDigits.get(d) ?? d + domainCadangan);
}

/**
 * @param {string[]|null} mentions Daftar JID lengkap yang ingin di-tag.
 *
 * WAJIB diisi kalau yang di-tag BUKAN si pengirim perintah — mis. daftar user
 * di `.listprem`. Tanpa itu, domain JID ditebak dari `senderType`, padahal itu
 * tipe PENGIRIM, bukan tipe orang yang di-tag. Akibatnya LID seperti
 * `225764989489187@lid` dikirim sebagai `...@s.whatsapp.net` dan WhatsApp
 * menampilkannya sebagai nomor telepon asing (`@+225 764989489187`), bukan tag.
 */
async function sendMessageWithMention(
  ...
  mentionedJid = Array.isArray(mentions)
    ? mentions.filter(Boolean)
    : await resolveMentionJids(sock, remoteJid, text, senderType);
...
async function sendMessageWithMentionNotQuoted(sock, remoteJid, text, senderType = null) {
  ...
  mentionedJid = await resolveMentionJids(sock, remoteJid, text, senderType);
...
/**
 * Kirim video/GIF dengan mention (dipakai welcome bermedia video).
 * Bentuknya sengaja sama dengan sendImagesWithMentionNotQuoted.
 *
 * @param {boolean} gifPlayback true HANYA untuk GIF. Kalau dipasang pada video
 *   biasa, WhatsApp memutarnya sebagai GIF: berulang & tanpa suara.
 */
async function sendVideoWithMentionNotQuoted(
  sock,
  remoteJid,
  buffer,
  text,
  ...
  const mentionedJid = await resolveMentionJids(sock, remoteJid, text, senderType);
...
async function checkIfAdmin(sock, remoteJid, sender) {
  try {
    ...
    // Lewat cache bersama (dedup + jeda kegagalan), bukan query langsung.
    const { getGroupMetadata, pesertaAdalahAdmin } = await import('./cache.js');
    const groupMetadata = await getGroupMetadata(sock, remoteJid);

    // FIX: participants validation - groupMetadata bisa null/incomplete
    // Pencocokan lewat semua bentuk identitas peserta (id/jid/lid/phoneNumber),
    // supaya admin di grup ber-alamat LID tidak terbaca sebagai user biasa.
    return pesertaAdalahAdmin(groupMetadata?.participants, sender);
  } catch (err) {
...
/** Pola URL longgar (skema & www opsional) — dipakai semua fitur antilink. */
const POLA_URL = /(?:https?:\/\/)?(?:www\.)?[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}(?:\/[^\s]*)?/g;

/**
 * Link undangan GRUP dan SALURAN WhatsApp.
 *
 * Keduanya sudah punya penangan sendiri (`antilinkwa` & `antilinkch`), jadi
 * `antilink` umum harus melewatkannya — kalau tidak, menyalakan `antilink`
 * saja sudah ikut menghapus link grup/saluran dan setelan antilinkwa/antilinkch
 * jadi tidak ada artinya.
 */
const POLA_LINK_WHATSAPP =
  /^(?:https?:\/\/)?(?:www\.)?(?:chat\.whatsapp\.com|whatsapp\.com\/channel)(?:\/|$)/i;

/** Semua URL di dalam teks. */
function daftarUrl(str) {
  if (typeof str !== 'string') return [];
  return str.match(POLA_URL) ?? [];
}

/** Apakah URL ini link undangan grup atau saluran WhatsApp? */
function isLinkGrupAtauSaluranWa(url) {
  return typeof url === 'string' && POLA_LINK_WHATSAPP.test(url);
}

function isUrlInText(str) {
  // FIX: startup match error - validasi string sebelum test()
  if (typeof str !== 'string') return false;
  return daftarUrl(str).length > 0;
}

/**
 * Ada URL SELAIN link grup/saluran WhatsApp?
 *
 * Teks yang hanya berisi link grup/saluran bernilai `false`, tapi teks yang
 * mencampur keduanya ("chat.whatsapp.com/xxx dan situs.com") tetap `true` —
 * link non-WhatsApp-nya tetap harus kena antilink.
 */
function isUrlInTextSelainWa(str) {
  return daftarUrl(str).some((url) => !isLinkGrupAtauSaluranWa(url));
}
  ...
  return new Promise((resolve, reject) => {
    ...
    archive.glob('**/*', {
      ...
      ignore: [
        ...
        'logs/**',
        'node_modules/**',
        '**/node_modules/**', // node_modules bersarang (mis. dashboard/) ikut dibuang
        '**/*.zip', // termasuk file backup lama, agar zip tidak menggelembung
      ],
    });
