...
function deleteMedia(fileName) {
  ...
  // Basename mencegah path traversal (mis. '../../session/creds.json')
  const safeName = path.basename(String(fileName || ''));
  const filePath = path.join(mediaFolder, safeName); // Gabungkan path folder dan nama file
  ...
  if (fs.existsSync(filePath)) {
    ...
    fs.unlink(filePath, (err) => {
      ...
      console.log(`File ${safeName} berhasil dihapus.`);
    });
  } else {
    console.log(`File ${safeName} tidak ditemukan di folder ${mediaFolder}.`);
  }
}
...
async function sendMessageWithMention(sock, remoteJid, text, message, senderType = null) {
  ...
  if (senderType === 'user') {
    mentionedJid = [...text.matchAll(/@(\d{1,16})/g)].map((match) => match[1] + '@s.whatsapp.net');
  } else if (senderType === 'lid') {
    mentionedJid = [...text.matchAll(/@(\d{1,16})/g)].map((match) => match[1] + '@lid');
  } else {
...
async function checkAndInstallModule(moduleName, version = null) {
  ...
  try {
    ...
    if (!mod) {
      ...
      // coba ambil versi dari package.json modul yang terinstal
      try {
        const modulePkgPath = path.join(process.cwd(), 'node_modules', moduleName, 'package.json');
        const modulePkg = JSON.parse(fs.readFileSync(modulePkgPath, 'utf8'));
        currentVersion = modulePkg.version || null;
      } catch {
        currentVersion = null;
      }
...
function flush() {
  ...
  // Persist ke file tracking (sebelumnya flushLogs tidak pernah dipanggil)
  flushLogs();
