...
/**
 * Tulis fitur ke DB hanya kalau ensureAllFeatures() menambah fitur baru.
 *
 * findGroup() dipanggil beberapa kali untuk SETIAP pesan grup. Dulu ia selalu
 * menjalankan UPDATE walau tidak ada yang berubah — penulisan database sia-sia
 * di tiap pesan yang ikut membebani CPU/disk panel.
 */
function simpanFiturJikaBerubah(id, fiturLamaJson, fiturBaru) {
  const json = toJson(fiturBaru);
  if (json === fiturLamaJson) return;
  getStmt('updateFitur', 'UPDATE groups_data SET fitur = ? WHERE id = ?').run(json, id);
}
...
async function findGroup(id, search = false) {
  try {
    if (search && id === 'owner') {
      ...
      simpanFiturJikaBerubah(id, row.fitur, data.fitur);
