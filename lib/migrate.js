...
function migrateUsers(db, data) {
  ...
  // ON CONFLICT DO UPDATE, bukan INSERT OR REPLACE: REPLACE menghapus baris
  // lama (termasuk yang bentrok UNIQUE username), sehingga satu tabrakan
  // username bisa menghilangkan record user lain.
  const stmt = db.prepare(`
    ...
    ON CONFLICT(id) DO UPDATE SET
      ...
      level_cache = excluded.level_cache,
  ...
  for (const [id, user] of Object.entries(data)) {
    try {
      ...
      console.log(`⚠️ Lewati user ${id} saat migrasi: ${error.message}`);
    }
  }
  return count;
}
...
function migrateBadwords(db, data) {
  ...
  for (const [id, badwordData] of Object.entries(data)) {
    if (!badwordData || typeof badwordData !== 'object') continue;
    stmt.run(
      id,
      toJson(badwordData),
