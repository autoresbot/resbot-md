...
import { getJadwalTeks } from './participants.js';
...
function waitForConnectionReady(sock, timeout = 15000) {
...
async function updateSocket(newSock) {
  ...
  // Job alarm ikut terhapus oleh pembatalan massal di atas, jadi harus
  // didaftarkan ulang setiap kali updateSocket dijalankan.
  syncAlarm(currentSock);
...
async function sendNotif(sock, groupId, waktu) {
  // Dicek saat waktunya tiba (bukan saat dijadwalkan), supaya .acc/.unacc
  // langsung berlaku tanpa restart.
  if (!isAccGroup(groupId)) return;
  ...
  try {
    ...
    try {
      await waitForConnectionReady(sock);
    } catch (err) {
    ...
    if (waktu == 'sahur') {
      ...
      // File bawaan & hasil upload dashboard adalah sahur.opus; sahur.m4a
      // dipertahankan sebagai cadangan untuk panel yang masih memakai file lama.
      const audioDir = path.join(process.cwd(), 'database', 'audio');
      const filePath = [path.join(audioDir, 'sahur.opus'), path.join(audioDir, 'sahur.m4a')].find(
        (file) => fs.existsSync(file),
      ) || path.join(audioDir, 'sahur.opus');
      try {
...
/**
 * Teks pengumuman buka/tutup grup.
 *
 * Admin bisa mengatur teksnya sendiri lewat `.setopengc 23:10 <teks>`; kalau
 * kosong dipakai teks bawaan. Variabel seperti @group ikut diganti supaya
 * teksnya bisa dipakai sama di banyak grup.
 */
function teksJadwal(groupId, property, bawaan, metadata) {
  const teks = getJadwalTeks(groupId, property);
  ...
  let hasil = teks;
  for (const [kunci, nilai] of Object.entries(pengganti)) {
    hasil = hasil.replace(new RegExp(kunci, 'gi'), nilai);
  }
  return hasil;
}

async function closeGroup(sock, groupId) {
  if (!isAccGroup(groupId)) return; // grup belum di-.acc
  try {
    const metadata = await getGroupMetadataSafe(sock, groupId);
    if (!metadata) return;
    ...
    await sock.sendMessage(groupId, {
      text: teksJadwal(groupId, 'closeText', mess.action.grub_close, metadata),
    });
  } catch (error) {
    await sock.sendMessage(groupId, {
...
async function openGroup(sock, groupId) {
  ...
  try {
    ...
    await sock.sendMessage(groupId, {
      text: teksJadwal(groupId, 'openText', mess.action.grub_open, metadata),
    });
  } catch (error) {
