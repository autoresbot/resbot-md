...
let updateSocketChain = Promise.resolve();

function updateSocket(newSock) {
  if (!STATUS_SCHEDULED) {
    return console.log('Scheduled tasks are disabled in config, skipping updateSocket');
  }

  const run = updateSocketChain.then(() => doUpdateSocket(newSock));
  updateSocketChain = run.then(
    () => undefined,
    () => undefined,
  );
  run.catch((error) => {
    console.error('updateSocket gagal:', error?.message || error);
  });
  return run;
}

async function doUpdateSocket(newSock) {
  ...
  try {
    ...
    for (const [waktu, jam] of Object.entries(dataSholat)) {
      ...
      for (const [groupIndex, groupId] of groupIds.entries()) {
        ...
        const staggerMs = groupIndex * delayBetweenNotif;
        ...
        schedule.scheduleJob(jobName, schedulePattern, async () => {
          setTimeout(async () => {
            try {
              ...
              console.error(`Gagal kirim notifikasi ${waktu} ke ${groupId}:`, error.message);
            }
          }, staggerMs);
        });
      }
...
async function restaringAction() {
  try {
    logWithTime('System', 'Restarting system otomatis: spawn proses baru lalu keluar');
    exec(`node index`);
    process.exit(0);
  } catch (error) {
