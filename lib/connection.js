 ...
 *
 * Nilainya BUKAN sekadar penanda "sudah pernah lewat", melainkan
 * `true/false` = salinan yang sudah diproses itu PUNYA ISI atau tidak.
 * Alasannya ada di `punyaIsi()` di bawah.
 */
const pesanDiproses = createBoundedMap({ max: 20000, ttl: 10 * 60 * 1000 });

/**
 * Apakah salinan pesan ini benar-benar membawa isi?
 *
 * WhatsApp kerap mengirim satu pesan DUA KALI dengan key.id yang sama:
 *   1. salinan pertama KOSONG - biasanya `senderKeyDistributionMessage`,
 *      yaitu pengirim membagikan kunci grupnya. Ini justru paling sering
 *      terjadi saat seseorang PERTAMA KALI chat di sebuah grup.
 *   2. salinan kedua berisi teks aslinya (mis. ".claim").
 *
 * Saringan pesan kembar yang cuma melihat ID akan meloloskan salinan KOSONG
 * lalu membuang salinan yang BERISI - command-nya tidak pernah dijalankan dan
 * gejalanya persis "chat pertama di grup tidak dibalas, harus beberapa kali".
 */
function hanyaKunciGrup(result) {
  // Jalur tag status (status@broadcast) sengaja dikecualikan: fitur antitagsw
  // memang bekerja dari senderKeyDistributionMessage.
  if (result?.remoteJid === 'status@broadcast' || result?.isTagSw) return false;

  const isi = result?.message?.message;
  if (!isi || typeof isi !== 'object') return false;

  const kunci = Object.keys(isi);
  if (!kunci.includes('senderKeyDistributionMessage')) return false;

  // Selain kunci grup, hanya pembungkus kosong yang boleh ikut.
  return kunci.every((k) => k === 'senderKeyDistributionMessage' || k === 'messageContextInfo');
}

function punyaIsi(result) {
  if (result?.fullText && result.fullText.trim()) return true;
  // Media tanpa caption tetap dihitung berisi.
  return Boolean(result?.type && result.type !== 'text' && result.type !== 'unknown');
}
...
import { createBoundedMap, createBoundedSet } from './boundedStore.js';
import { getJadibot } from './jadibot.js';
import { chatMasuk, chatBerhenti, chatError, chatTahap, teksMentah } from './trace.js';
...
async function connectToWhatsApp(folder = 'session') {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    try {
      // Jejak paling awal: dicetak SEBELUM apa pun diproses, jadi kalau baris
      // ini tidak muncul berarti pesannya memang tidak pernah sampai ke bot
      // (masalah koneksi/dekripsi WhatsApp), bukan tersangkut di kode.
      const mentah = m?.messages?.[0];
      const jejakAwal = {
        id: mentah?.key?.id,
        remoteJid: mentah?.key?.remoteJid,
        pushName: mentah?.pushName,
        teks: teksMentah(mentah),
      };
      if (mentah && m?.type !== 'append') chatMasuk(jejakAwal);

      // Satu event 'messages.upsert' BISA membawa beberapa pesan sekaligus,
      // tapi kode di bawah hanya memproses messages[0]. Sisanya hilang tanpa
      // jejak. Dicatat di sini supaya ketahuan kalau inilah penyebab pesan
      // yang "tidak masuk".
      if (Array.isArray(m?.messages) && m.messages.length > 1 && m?.type !== 'append') {
        chatBerhenti(
          'Batch pesan',
          `event ini membawa ${m.messages.length} pesan, tapi hanya pesan ke-1 yang diproses (${m.messages.length - 1} pesan dilewati)`,
          jejakAwal,
        );
      }
      ...
      if (!result) {
        if (mentah && m?.type !== 'append') {
          chatBerhenti('Baca pesan', 'pesan tidak bisa dibaca / disaring (lihat baris di atas)', jejakAwal);
        }
        return;
      }
      ...
      const adaIsi = punyaIsi(result);
      const isiSalinanLama = result.id ? pesanDiproses.get(kunciPesan) : undefined;

      if (result.id && isiSalinanLama !== undefined) {
        // Kembar sungguhan: salinan lama sudah berisi, ATAU yang ini pun kosong.
        if (isiSalinanLama === true || !adaIsi) {
          chatBerhenti(
            'Saringan pesan kembar',
            isiSalinanLama === true
              ? 'pesan dengan ID sama sudah diproses'
              : 'salinan kosong, sudah ada salinan lain dengan ID sama',
            result,
          );
          return;
        }

        // Salinan lama KOSONG dan yang ini BERISI -> inilah pesan sebenarnya.
        chatTahap('Kiriman ulang dipakai - salinan pertama kosong (kunci grup)', result);
      }

      if (result.id) pesanDiproses.set(kunciPesan, adaIsi);

      // Salinan "kunci grup" murni: tidak ada teks, tidak ada media, isinya
      // cuma senderKeyDistributionMessage. Tidak ada yang bisa dikerjakan
      // plugin/handler dari pesan ini, jadi dihentikan di sini supaya tidak
      // membebani pipeline dan tidak bikin log jadi membingungkan.
      // ID-nya SUDAH dicatat di atas sebagai "kosong", jadi salinan berisi
      // yang menyusul tetap akan diproses.
      if (!adaIsi && hanyaKunciGrup(result)) {
        chatBerhenti(
          'Pesan kunci grup',
          'salinan pembagian kunci (tanpa isi) - menunggu salinan aslinya',
          result,
        );
        return;
      }
      ...
      try {
        ...
        if (presenceValid && !grupDiam) {
          // Dibungkus sendiri: kalau presence gagal (socket sedang tersendat),
          // dulu error-nya melompat ke catch di bawah sehingga processMessage()
          // TIDAK PERNAH dipanggil - pesannya hilang tanpa alasan yang jelas.
          try {
            await sock.sendPresenceUpdate(config.PresenceUpdate, remoteJid);
          } catch (error) {
            logWithTime('System', `PresenceUpdate gagal: ${error?.message || error}`);
          }
        } else {
        ...
        chatError('Pemrosesan pesan', error, result);
...
// punyaIsi & hanyaKunciGrup ikut diekspor supaya logika saringan pesan
// kembar bisa diuji tanpa membuka koneksi WhatsApp sungguhan.
export { connectToWhatsApp, punyaIsi, hanyaKunciGrup };
