  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
      ...
      if (reconnectAttempts >= MAX_RECONNECT) {
        console.log(chalk.bgRed('Max reconnect reached. Stop reconnecting.'));
        return;
      }
      ...
      console.log(
        chalk.yellow(`Reconnect attempt ${reconnectAttempts}/${MAX_RECONNECT} | Reason: ${reason}`),
      );

      // khusus 403 protection
      if (reason === 403) {
        const now = Date.now();
        ...
        if (error403Timestamps.length > 3) {
          console.log(
            chalk.bgRed('[BLOCKED] Terlalu banyak error 403 dalam 1 menit. Stop reconnect.'),
          );
          reconnecting = false;
          return;
        }
        ...
        if (isSession) {
          ...
          if (sock?.ws?.readyState === 1 && sock?.user) {
            try {
              ...
              console.log('Restart message failed:', e.message);
            }
          }
        }
      } else if (isSession) {
        setTimeout(async () => {
          if (sock?.ws?.readyState === 1) {
            try {
              ...
              console.log('Send after open failed:', e.message);
            }
          }
        }, 5000);
      }
