...
async function connectToWhatsApp(folder = 'session') {
  ...
  sock.ev.on('messages.upsert', async (m) => {
    ...
    try {
      ...
      for (const singleMessage of m.messages) {
        // Pengelolaan Pesan Masuk pindah ke /lib/serializeMessage.js
        const result = serializeMessage({ ...m, messages: [singleMessage] }, sock);
        if (!result) {
          continue;
        }
...
_Website: autoresbot.com/contact_`;
  ...
  sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
      ...
      state_session.connectedAt = Date.now();
    ...
    if (connection === 'close') {
      ...
      // Koneksi yang stabil cukup lama dianggap periode baru: budget
      // reconnect di-reset agar gangguan sesekali tidak menghabiskan
      // jatah kumulatif dan mematikan auto-reconnect selamanya.
      const STABLE_THRESHOLD = 5 * 60 * 1000;
      if (
        state_session.connectedAt &&
        Date.now() - state_session.connectedAt > STABLE_THRESHOLD
      ) {
        state_session.attempts = 0;
      }
