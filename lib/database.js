...
function initDatabase() {
  ...
  // Dicek SEBELUM createTables(): tabel acc_groups yang baru pertama kali
  // dibuat berarti script baru saja di-update ke sistem .acc.
  const accGroupsBaru = !db
    .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'acc_groups'")
    .get();

  createTables();
  migrateSchema();

  // Sistem .acc membuat bot diam di semua grup yang belum diaktifkan owner.
  // Grup yang sudah punya sewa saat update diaktifkan otomatis SEKALI, supaya
  // pelanggan sewa lama tidak ikut mati.
  if (accGroupsBaru) {
    const hasil = db
      .prepare(
        `INSERT OR IGNORE INTO acc_groups (id, added_by, created_at)
         SELECT id, 'sewa', datetime('now') FROM sewa WHERE id LIKE '%@g.us'`,
      )
      .run();
    if (hasil.changes > 0) {
      console.log(`[✔] ${hasil.changes} grup sewa otomatis diaktifkan (.acc)`);
    }
  }
...
function createTables() {
  db.exec(`
    ...
    -- Tabel user yang sedang di-silent (.silent): pesannya dihapus bot
    CREATE TABLE IF NOT EXISTS silent (
      group_id TEXT NOT NULL,
      user_id TEXT NOT NULL,        -- nomor/LID tanpa suffix, untuk pencocokan
      until INTEGER,                -- timestamp ms; 0 = selamanya
      reason TEXT,
      created_by TEXT,
      created_at TEXT DEFAULT (datetime('now')),
    ...
    -- Tabel grup yang diaktifkan owner lewat .acc (bot diam di grup lain)
    ...
    -- Tabel Alarm (pesan terjadwal per grup)
    ...
    -- Nama alarm unik per grup: .addalarm dengan nama yang sama = memperbarui
    CREATE UNIQUE INDEX IF NOT EXISTS idx_alarms_group_name ON alarms(group_id, name);
  `);
}
