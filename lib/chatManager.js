...
const MAX_USERS = 2000; // batas jumlah user agar map tidak tumbuh tanpa batas
...
function addChat(userId, message) {
  ...
  while (chatHistoryMap.size > MAX_USERS) {
    const oldestUser = chatHistoryMap.keys().next().value;
    chatHistoryMap.delete(oldestUser);
  }
}
