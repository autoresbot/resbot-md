...
function pecahRantai(messageInfo, plugins = []) {
  ...
  // Command PERTAMA harus bisa dirantai. Kalau tidak, " + " hampir pasti cuma
  // bagian dari teks biasa (mis. `.brat azhari imut + cantik`,
  // `.ai berapa 1 + 1`) - biarkan diproses seperti biasa.
  if (!aturanRantai(bagian[0].split(/\s+/)[0])) return null;

  // Setiap potongan sesudahnya harus diawali nama command yang dikenal.
  // `.smeme halo + dunia` -> "dunia" bukan command, jadi itu teks biasa.
  const semuaCommand = bagian.slice(1).every((b) => commandTerdaftar(plugins, b.split(/\s+/)[0]));
  if (!semuaCommand) return null;
  ...
  for (const potongan of bagian) {
    ...
    if (!aturan) {
      // Sampai sini cmd pasti command terdaftar (sudah dicek di atas), hanya
      // saja belum didukung dalam rantai (mis. `.removebg + brat`).
      return {
