...
function serializeMessage(m, sock) {
  try {
    ...
    // sebelum proses parsing command, agar "h tes", "h  tes", "h   tes" tetap terdeteksi.
    content = (content || '').replace(/\s+/g, ' ').trim();

    // Pertahankan dukungan prefix diikuti spasi (mis. ". on" -> ".on")
    const prefixWithSpace = config.prefix.find((p) => content.startsWith(p + ' '));
    if (prefixWithSpace) {
      content = prefixWithSpace + content.slice(prefixWithSpace.length + 1);
    }

    const parts = content.split(' '); // content sudah dinormalisasi, aman di-split per satu spasi
    ...
    const contentWithoutCommand = parts.slice(1).join(' ').trim();
