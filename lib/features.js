...
async function getJadwalSholat(kota = 'jakarta') {
  try {
    // Jika data sudah di-cache untuk hari ini, kembalikan data tersebut
    const todayKey = getWibDateKey();
    if (cachedJadwalSholat && cachedJadwalDate === todayKey) {
...
async function textToAudio(text) {
  try {
    ...
    const url = `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(truncatedText)}&tl=id&client=tw-ob`;
