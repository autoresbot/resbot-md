...
const CACHE_groupFetch = 10 * 60000; // 10 menit (60000 ms)

// Timer penghapus per JID: dibatalkan saat cache di-refresh agar timer lama
// tidak menghapus entri baru yang belum kedaluwarsa.
const groupMetadataTimers = {};

function setGroupCacheEntry(jid, metadata) {
  clearTimeout(groupMetadataTimers[jid]);
  groupCache[jid] = {
    ...metadata,
    last_update: Date.now(),
  };
  groupMetadataTimers[jid] = setTimeout(() => {
    delete groupCache[jid];
    delete groupMetadataTimers[jid];
  }, CACHE_METADATA);
}
...
const getGroupMetadata = async (sock, remoteJid) => {
  ...
  if (!groupCache[remoteJid]) {
    try {
      ...
      setGroupCacheEntry(remoteJid, metadata);
    } catch (err) {
...
const getProfilePictureUrl = async (sock, sender) => {
  ...
  if (cached) {
    ...
    // Entri kedaluwarsa dibuang agar cache tidak tumbuh tanpa batas
    delete profilePictureCache[sender];
  }
...
const clearGroupCache = (remoteJid) => {
  ...
  clearTimeout(groupMetadataTimers[remoteJid]);
  delete groupMetadataTimers[remoteJid];
};
