...
/**
 * Cari JID peserta grup dari identitas apa pun (nomor telepon / LID, dengan
 * atau tanpa penanda device). Mengembalikan null kalau peserta tidak ketemu.
 *
 * Dipakai untuk MENTION. `contextInfo.participant` pada pesan yang di-quote
 * bisa datang sebagai LID ATAU nomor telepon, tergantung versi WhatsApp si
 * pengirim — padahal tag hanya tampil sebagai nama kalau teks `@nomor` dan JID
 * yang di-mention memakai bentuk yang SAMA dengan peserta grup (di grup
 * ber-alamat LID itu `@lid`). Tanpa penyeragaman ini, mention berbentuk nomor
 * telepon di grup ber-alamat LID muncul sebagai nomor asing, bukan tag.
 */
const jidPesertaGrup = (participants, identitas) => {
  const target = nomorDari(identitas);
  if (!target) return null;
  const gp = (participants || []).find((p) => p && pesertaCocok(p, target));
  if (!gp) return null;
  return gp.id || gp.jid || gp.lid || gp.phoneNumber || null;
};
