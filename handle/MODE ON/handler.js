...
import { devlog, isDevLogEnabled } from '../../lib/devlog.js';
  ...
  try {
    ...
    if (isDevLogEnabled()) {
      devlog('admin', {
        ...
        senderLid,
        isAdmin,
        // Kandidat yang cocok sebagian: kalau ini terisi tapi isAdmin false,
        // berarti pencocokan jid-nya benar tapi field admin-nya yang salah.
        cocokSender: participants
          .filter((p) => p.phoneNumber === sender || p.id === senderLid)
          .map((p) => ({ id: p.id, phoneNumber: p.phoneNumber, admin: p.admin })),
        totalPeserta: participants.length,
        contohPeserta: participants
          .slice(0, 3)
          .map((p) => ({ id: p.id, phoneNumber: p.phoneNumber, admin: p.admin })),
      });
    }
    ...
    const deleteMessage = async () => {
      ...
      const target = { remoteJid, id, participant: senderLid, fromMe: false };
      try {
        const result = await sock.sendMessage(remoteJid, { delete: target });
        devlog('handler', { aksi: 'deleteMessage', ok: true, command, target, hasil: result?.key?.id });
      } catch (error) {
        devlog('handler', {
          aksi: 'deleteMessage',
          ok: false,
    ...
    if (isDevLogEnabled()) {
      // Kalau antilink "tidak jalan", jawabannya ada di sini: apakah teksnya
      // terbaca, apakah fitur menyala, dan apakah pengirim dianggap admin
      // (admin sengaja dikecualikan dari semua antilink).
      devlog('handler', {
        aksi: 'cekAntilink',
        remoteJid,
        command,
        ...
        isWhatsappSaluran,
        fitur: {
          antilink: fitur.antilink,
          antilinkv2: fitur.antilinkv2,
          antilinkwa: fitur.antilinkwa,
          antilinkwav2: fitur.antilinkwav2,
          antilinkch: fitur.antilinkch,
          antilinkchv2: fitur.antilinkchv2,
        },
      });
    }
